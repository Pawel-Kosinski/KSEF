...
async def spending_trend(
    ...
    category: str | None = Query(
        default=None,
        description="Opcjonalny filtr kategorii (nazwa z ai_category_main)",
    ),
    service: StatisticsService = Depends(_statistics_service),
) -> TrendResponse:
...
async def cashflow(
    ...
    return await service.get_cashflow(date_from=date_from, date_to=date_to)
...
async def dashboard_bundle(
    ...
    return await service.get_dashboard(role=role, date_from=date_from, date_to=date_to)
